  ...
  const createTable = async (e) => {
    try {
      ...
      const response = await axios.post(
        ...
        {
          headers: headers,
          num_mesa: parseInt(e.target.value),
        }
      );
  ...
  return (
    <section>
      <header class="bg-white space-y-4 p-4 sm:px-8 sm:py-6 lg:p-4 xl:px-8 xl:py-6">
        <form onSubmit={handleSubmit} class="group relative">
          <svg
            width="20"
            height="20"
            fill="currentColor"
      ...
      <ul class="bg-white p-4 sm:px-8 sm:pt-6 sm:pb-8 lg:p-4 xl:px-8 xl:pt-6 xl:pb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 text-sm leading-6">
        {tables
